...
@mysql_select_db($database) or die("Unable to select database");
	...
	if (($projectid != "") && ($frombranchid != ""))
	{
        ...
        if ($frombranchid!=$headid) {
            ...
            $sql = "SELECT id, name FROM tbbranch WHERE visible=1 and project_id=$projectid and (  ((versionnr>=$versionnr) and (istag=0)) or ((versionnr>$versionnr) and (istag=1)) ) order by istag, name asc;";
